...
import argparse, glob, os, gymnasium as gym , importlib
...
def main():
    ...
    for algo in args.algos:
        ...
        row = dict(task=args.task, algo=algo, episodes=args.episodes,
                   checkpoint=os.path.dirname(ckpt), **metrics)
        
        all_rows.append(row)

        print(f"{algo:7s} | R {metrics['reward_mean']:7.2f} ± {metrics['reward_std']:5.2f} | "
