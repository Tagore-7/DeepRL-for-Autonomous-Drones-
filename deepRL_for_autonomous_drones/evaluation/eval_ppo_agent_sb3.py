...
# @dataclass
# class EvalConfig:
#     model_path: str = "../training/benchmark_results/add_seeding_trees/best_model"
#     task: str = "SafetyDroneLanding-v0"
#     # wind_levels: tuple = ("none", "light_breeze", "light", "medium", "high")
#     wind_levels: tuple = ("none")
...
#     env = DummyVecEnv([lambda: make_env(cfg.task, "none", render=cfg.render)])

#     ep_rewards, ep_lengths, ep_costs = [], [], []

#     for _ in trange(cfg.eval_episodes):
#         obs = env.reset()
#         done = False
#         total_reward = 0.0
#         total_cost = 0.0
...
def make_env(task: str, render: bool = False, seed: int = 42, video_dir: str = None):
    render_mode = "human" if render else None
    # render_mode = "rgb_array" if video_dir else ("human" if render else None)
    env = gym.make(task, render_mode=render_mode)

    # if video_dir is not None:
    #     env = RecordVideo(env, video_folder=video_dir, episode_trigger=lambda ep_id: True, name_prefix="eval")  # record every episode
    env = FlattenObservation(env)
    env.reset(seed=seed)
    return env
...
def eval_agent(model_path: str, env_name: str, render: bool = False, episodes: int = 20):
    ...
    env = make_env(env_name, render, base_seed, video_dir="./videos")
...
if __name__ == "__main__":
    eval_agent(
        # model_path="../training/saved_models/Sept_24/sensor_test_2/best_model",
