...
class Sensor(abc.ABC):
    ...
    def __init__(self, bc, offset: list, agent, obstacles: list, coordinate_system: int, rotate_with_agent: bool, visualize: bool):
        self.agent = agent
        self.bc = bc
...
class LIDARSensor(Sensor):
    ...
    def init_rays(self) -> list:
        ...
        ray_ids = []
        for _ in range(self.number_rays):
            if self.replace_lines and self.visualize:
                end_point = [1, 1, 1]
                ray_ids.append(self.bc.addUserDebugLine(from_position, end_point, self.miss_color, lineWidth=self.ray_width))
            else:
                ray_ids.append(-1)

        return ray_ids

    def set_ray_positions(self, from_position=None, shift=0.0):
        ...
        for i in range(self.number_rays):
            ...
            angle = (2.0 * math.pi * float(i)) / self.number_rays + shift
    ...
    def render(self, data) -> None:
        ...
        for i in range(self.number_rays):
            ...
            if hitObjectUid < 0:  # no object intersection
                ...
                self.bc.addUserDebugLine(
                    self.rays_starting_points[i], hitPosition, self.hit_color, lineWidth=self.ray_width, replaceItemUniqueId=self.ray_ids[i]
                )
...
class PseudoLIDARSensor(LIDARSensor):
    ...
    def __init__(self, bc, agent, obstacles, number_rays, ray_length, offset=(0, 0, 0), coordinate_system=pb.WORLD_FRAME, visualize=True):
    ...
    def render(self, hit_distances: np.ndarray) -> None:
        ...
        for i in range(self.number_rays):
            ...
            self.bc.addUserDebugLine(
                self.rays_starting_points[i], self.rays_end_points[i], color, lineWidth=self.ray_width, replaceItemUniqueId=self.ray_ids[i]
            )
