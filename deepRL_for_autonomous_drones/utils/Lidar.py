...
class Lidar():
    ...
    def CheckHits(self, ray_from_position, ray_orientation, ray_length, offset, draw_debug_line=False):
        """
        ...
        #---- Perform ray casting ----#
        hit_results = p.rayTestBatch(ray_from, ray_to, numThreads=0)
        ...
        if draw_debug_line:
            if self.replace_item_uniqueIds is None:
                ...
                for idx, res in enumerate(hit_results):
                    if res[0] != -1:
                        p.addUserDebugLine(ray_from[idx], res[3], lineColorRGB=[1, 0, 0],
                                            replaceItemUniqueId=self.replace_item_uniqueIds[idx])
                    else:
                        p.addUserDebugLine(ray_from[idx], ray_to[idx], lineColorRGB=[0, 1, 0],
                                            replaceItemUniqueId=self.replace_item_uniqueIds[idx])
        return hit_results